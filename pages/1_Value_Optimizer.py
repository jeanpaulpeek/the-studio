# ============================================
# value_optimizer.py — G/B/B + vergelijking + export
# ============================================
...
def _on_page(letterhead_img_reader=None, footer_text=""):
    # Tekent optioneel briefpapier (PNG/JPG) + footer/paginanr
    from reportlab.pdfgen import canvas as rl_canvas

    def _cb(c: rl_canvas.Canvas, doc):
        if letterhead_img_reader is not None:
            w, h = PAGE_SIZE
            c.drawImage(
                letterhead_img_reader,
                0,
                0,
                width=w,
                height=h,
        ...
        c.drawRightString(
            ...
            10 * mm,
            f"{footer_text}  •  pagina {c.getPageNumber()}",
        )
...
def build_offer_pdf(
    project_meta: dict,
    result_df,  # pd.DataFrame (regels met artikelen)
    summary_df,  # pd.DataFrame (samenvatting per soort)
    show_prices: bool = True,
    letterhead_image_bytes: bytes | None = None,  # alleen bij PNG/JPG
    internal: bool = False,
) -> bytes:
    """Bouwt een A4-PDF. Bij letterhead_image_bytes wordt het afbeeldings-briefpapier
       paginavullend gezet op elke pagina (geschikt voor PNG/JPG)."""
    buf = io.BytesIO()
    ...
    # Zorg voor bruikbare presentiekolommen
    ...
    footer = project_meta.get("footer", "© jouw-merk – gegenereerd met Quote Optimizer")
    img_reader = ImageReader(io.BytesIO(letterhead_image_bytes)) if letterhead_image_bytes else None
    doc.build(
        story,
        onFirstPage=_on_page(img_reader, footer),
        onLaterPages=_on_page(img_reader, footer),
    )
    return buf.getvalue()


def overlay_pdf_letterhead(base_pdf_bytes: bytes, letterhead_pdf_bytes: bytes, first_page_only=True) -> bytes:
    """Legt een PDF-briefpapier bovenop de gegenereerde PDF en schaalt het naar de pagina."""
    ...
    for i, page in enumerate(base_reader.pages):
        ...
        if (not first_page_only) or (first_page_only and i == 0):
            if Transformation is not None:
                ...
                # Belangrijk: pak telkens een NIEUW overlay-object (pagina 0)
                ...
                # Fallback: merge zonder schalen (verwacht A4 op A4)
                overlay_page = PdfReader(io.BytesIO(letterhead_pdf_bytes)).pages[0]
...
# (optioneel thema/navigatie – faalt stil als module niet bestaat)
try:
    from ui_theme import inject_theme, render_nav_with_icons
except Exception:
    def inject_theme(): ...
    def render_nav_with_icons(): ...
...
source_choice = st.radio(
    "Bron", ["Demo-prijslijst gebruiken", "Upload eigen prijslijst"], index=0, horizontal=True
)
...
def _normalize_pricelist(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "korting_leverancier" in df.columns:
        df["korting_leverancier"] = (
            pd.to_numeric(df["korting_leverancier"], errors="coerce").fillna(0.0)
        )
...
with colK:
    klant_korting_pct = st.number_input(
        "Klantkorting (%)", min_value=0.0, max_value=100.0, value=0.0, step=1.0
    )
...
st.markdown(
    '<div class="step-label">STAP 4. INVULLEN PER SOORT (AANTAL / KLASSEN / NIET MIXEN)</div>',
    unsafe_allow_html=True,
)
...
with st.form("opt_form", clear_on_submit=False):
    ...
    # (3) DOELKLASSE vetgedrukt bij doel=klasse
    target_avg = None
    if doel == DOEL_KLASSE:
...
if submitted:
    ...
    needs = {
        row["soort"]: int(max(0, row["aantal"]))
        for _, row in ed.iterrows()
        if (bool(row["actief"]) or int(row.get("aantal", 0)) > 0)
    }
    per_type_min = {
        row["soort"]: float(row.get("min_klasse", 0.0) or 0.0) for _, row in ed.iterrows()
    }
    per_type_max = {
        row["soort"]: float(row.get("max_klasse", 0.0) or 0.0) for _, row in ed.iterrows()
    }
    no_mix_by_type = {
        row["soort"]: bool(row.get("niet_mixen", False)) for _, row in ed.iterrows()
    }
...
if "gbb_options" in st.session_state and st.session_state["gbb_options"]:
    ...
    # Orden: GOED, ALT_A, ALT_B
    by_role = {o["role"]: o for o in opts}
    good = by_role.get("GOOD", opts[0])
    altA = by_role.get("ALT_A", None)
    altB = by_role.get("ALT_B", None)
...
def _build_grp_from_selection(sel_df: pd.DataFrame) -> pd.DataFrame:
    ...
    grp["gem_klasse_per_soort"] = grp.apply(
        lambda r: (r["som_gew_klasse"] / r["som_aantal"]) if r["som_aantal"] > 0 else 0.0, axis=1
    )
...
if not opts:
    ...
    for i, o in enumerate(opts):
        if i == 0:
            ...
            labels.append("ALTERNATIEF B")
        else:
            labels.append(o.get("name", f"Optie {i+1}"))
    ...
    # Prijsvelden (fallback op 'prijs' als 'price_effective' mist)
    ...
    # Alleen gekozen regels (>0) en subtotalen
    ...
    # Compacte weergave
    show = sel_df[
        ["soort", "artikel", "merk", "aantal", "price_effective", "omzet_subtotaal"]
    ].rename(
        columns={
            "soort": "Soort",
            "artikel": "Artikel",
    ...
    st.dataframe(
        show.style.format(
            {"Prijs/stuk (klant)": _euro0, "Subtotaal (klant)": _euro0}
        ),
        use_container_width=True,
        hide_index=True,
    )
    ...
    with cA:
        st.metric("Totaal (klant)", _euro0(total_omzet))
    with cB:
        st.metric("Gemiddelde klasse", f"{avg_klasse:.2f}".replace(".", ","))
    with cC:
        st.metric("Marge (intern)", f"{_euro0(marge_eur)}  ·  {marge_pct:.1f}%")

    st.caption(f"**Totaal stuks:** {int(show['Aantal'].sum()) if not show.empty else 0}")

    # ----- grp (samenvatting per soort) voor PDF -----
    grp = _build_grp_from_selection(sel_df)
    st.session_state["current_sel_df"] = sel_df.copy()
    st.session_state["current_grp"] = grp.copy()

    # ==== PDF export (compleet en met briefpapier) ====
    st.markdown("### PDF EXPORT")
    c1, c2 = st.columns([2, 1])
    with c1:
        project_title = st.text_input(
            "Titel op de offerte/rapport",
            value=st.session_state.get("pdf_title", ""),
            key="pdf_title",
        )
        project_client = st.text_input(
            "Klantnaam (optioneel)",
            value=st.session_state.get("pdf_client", ""),
            key="pdf_client",
        )
        project_ref = st.text_input(
            "Referentie (optioneel)",
            value=st.session_state.get("pdf_ref", ""),
            key="pdf_ref",
        )
    with c2:
        pdf_variant = st.radio(
            "PDF-type", ["Offerte (klant)", "Rapport (intern)"], horizontal=True, index=0, key="pdf_variant_radio"
        )
        only_first_page = st.checkbox(
            "Alleen 1e pagina bij PDF-briefpapier",
            value=st.session_state.get("pdf_firstpage", True),
            key="pdf_firstpage",
        )

    internal = pdf_variant == "Rapport (intern)"
    letterhead_file = st.file_uploader(
        "Optioneel: upload briefpapier (PNG, JPG of PDF)", type=["png", "jpg", "jpeg", "pdf"], key="pdf_letterhead"
    )
    ...
    if st.button("📄 Genereer PDF", key="pdf_generate"):
        ...
        # Maak presentiekolommen (zeker aanwezig)
        if "prijs" not in sel_df_pdf.columns and "price_effective" in sel_df_pdf.columns:
            sel_df_pdf["prijs"] = sel_df_pdf["price_effective"]  # fallback
        ...
        for c in ["omzet", "inkoop", "marge", "marge_pct", "gem_klasse_per_soort"]:
            if c in pdf_grp.columns:
                pdf_grp[c] = pdf_grp[c].round(2)

        # ---- Briefpapier routering
        final_pdf = None
        ...
        # Afb-briefpapier (PNG/JPG) -> direct in build_offer_pdf
        ...
        # PDF-briefpapier -> eerst basis bouwen, dan overlayen
        else:
            base_pdf = build_offer_pdf(
                project_meta=meta,
                result_df=sel_df_pdf,
...
st.button(
    "RESET INVOER",
    help="Zet aantallen/keuzes/resultaten terug; behoudt huidige prijslijst-keuze.",
    on_click=_soft_reset,
)
